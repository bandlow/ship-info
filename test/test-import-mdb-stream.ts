// test/test-import-mdb-stream.ts
import cds from '@sap/cds';
import { MDBStreamImporter } from '../srv/importers/mdb-stream-importer-v2.js';
...
const LOG = cds.log('test-mdb-stream');
const { SELECT } = cds.ql;
...
async function testMDBStreamImport() {
  try {
    LOG.info('🚀 Starting MDB Stream Import Test');
    logMemory('Start');
    ...
    // MDB Stream Import
    LOG.info('');
    LOG.info('=== MDB Stream Import ===');
    
    const importer = new MDBStreamImporter();
